...
export default function Reader({ book, chapters, chapterNum, onChapterChange, onFlashcardsChange, categories, onCategoriesChange, showPhonemeHints, selectedVoice, selectedLang, ttsRate = 0.9, onRateToggle, onRateChange, isMobile = false, onOpenSidebar, voices = [], voiceName = '', onVoiceChange, allVoices = [], langVoiceNames = {}, getVoiceForLang, onLangVoiceChange, languages = [], onLangChange, onPhonemeToggle: _onPhonemeToggle, theme, onThemeChange }: Props) {
  ...
  const [mnemonicCandidates, setMnemonicCandidates] = useState<Record<string, string[]>>({});
  const [mnemonicChoices, setMnemonicChoices] = useState<Record<string, string>>(() => {
    try { return JSON.parse(localStorage.getItem('mnemonic-choices') ?? '{}') as Record<string, string>; }
    catch { return {}; }
  });
  ...
  useEffect(() => {
    fetch('/mnemonic-candidates.json')
      .then(r => r.ok ? r.json() as Promise<Record<string, string[]>> : {})
      .then(setMnemonicCandidates)
      .catch(() => {});
  }, []);

  const saveMnemonicChoice = useCallback((word: string, anchor: string) => {
    setMnemonicChoices(prev => {
      const next = { ...prev, [word]: anchor };
      localStorage.setItem('mnemonic-choices', JSON.stringify(next));
      return next;
    });
  }, []);

  const clearMnemonicChoice = useCallback((word: string) => {
    setMnemonicChoices(prev => {
      const { [word]: _removed, ...rest } = prev;
      localStorage.setItem('mnemonic-choices', JSON.stringify(rest));
      return rest;
    });
  }, []);
            ...
            return (
              <div key={li} className={li > 0 ? 'mt-3' : ''}>
                ...
                {showLine0 && isCurrent && (() => {
                  ...
                  // User-chosen anchor from candidate selection
                  if (mnemonicChoices[lower]) {
                    return (
                      <div className="mb-0.5" onClick={e => e.stopPropagation()}>
                        <div className="font-mono text-sm leading-snug text-violet-300 italic flex items-center gap-2 flex-wrap">
                          <span className="not-italic font-bold text-violet-500 text-xs">{active.token.rawWord} → </span>
                          <span>{mnemonicChoices[lower]}</span>
                          <button
                            ...
                            onClick={() => clearMnemonicChoice(lower)}
                  ...
                  // Candidate selection list (generated by gen_mnemonic_candidates.py)
                  const candidates = mnemonicCandidates[lower];
                  if (candidates?.length) {
                    const saveCustom = (key: string) => {
                      const v = mnemonicCustomInput.trim();
                      if (v) { saveMnemonicChoice(key, v); setMnemonicCustomInput(''); }
                    };
                    return (
                      <div className="mb-0.5" onClick={e => e.stopPropagation()}>
                        <div className="text-[10px] text-violet-500/60 font-mono mb-1.5">
                          <span className="font-bold text-violet-400">{active.token.rawWord}</span> → pick sound-alike:
                        </div>
                        <div className="flex items-center gap-1 mb-1.5">
                          <input
                            type="text"
                            value={mnemonicCustomInput}
                            onChange={e => setMnemonicCustomInput(e.target.value)}
                            onKeyDown={e => { e.stopPropagation(); if (e.key === 'Enter') saveCustom(lower); if (e.key === 'Escape') setMnemonicCustomInput(''); }}
                            placeholder="type your own…"
                            className="text-xs px-2 py-0.5 rounded bg-violet-950/50 text-violet-200 border border-violet-500/30 focus:border-violet-400 outline-none flex-1 min-w-0"
                          />
                          <button onClick={() => saveCustom(lower)} title="Save" className="text-xs px-1.5 py-0.5 rounded bg-violet-500/20 text-violet-300 hover:bg-violet-500/40 border border-violet-500/30 transition-colors">&#10003;</button>
                          <button onClick={() => setMnemonicCustomInput('')} title="Clear" className="text-xs px-1.5 py-0.5 rounded bg-slate-700/30 text-slate-400 hover:bg-slate-700/60 border border-slate-600/30 transition-colors">&#10007;</button>
                        </div>
                        <div className="flex flex-wrap gap-1">
                          {candidates.map(c => (
                            <button
                              key={c}
                              onClick={() => saveMnemonicChoice(lower, c)}
                  ...
                  return (
                    <div className="mb-0.5" onClick={e => e.stopPropagation()}>
                      ...
                      {partItems.map(({ form, key, kind }, i) => {
                        ...
                        const saveContrCustom = () => { const v = mnemonicCustomInput.trim(); if (v) { saveMnemonicChoice(key, v); setMnemonicCustomInput(''); } };
                        return (
                          <div key={i}>
                            <div className="text-[10px] text-violet-500/60 font-mono mb-1.5">
                            ...
                            <div className="flex items-center gap-1 mb-1.5">
                              <input
                                ...
                                value={mnemonicCustomInput}
                                onChange={e => setMnemonicCustomInput(e.target.value)}
                                onKeyDown={e => { e.stopPropagation(); if (e.key === 'Enter') saveContrCustom(); if (e.key === 'Escape') setMnemonicCustomInput(''); }}
                                placeholder="type your own…"
                                className="text-xs px-2 py-0.5 rounded bg-violet-950/50 text-violet-200 border border-violet-500/30 focus:border-violet-400 outline-none flex-1 min-w-0"
                              />
                              <button onClick={saveContrCustom} title="Save" className="text-xs px-1.5 py-0.5 rounded bg-violet-500/20 text-violet-300 hover:bg-violet-500/40 border border-violet-500/30 transition-colors">&#10003;</button>
                              <button onClick={() => setMnemonicCustomInput('')} title="Clear" className="text-xs px-1.5 py-0.5 rounded bg-slate-700/30 text-slate-400 hover:bg-slate-700/60 border border-slate-600/30 transition-colors">&#10007;</button>
                            </div>
                            <div className="flex flex-wrap gap-1">
                              {cands.map(c => (
